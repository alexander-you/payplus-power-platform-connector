...
## ייבוא מסופים ועמודי תשלום נכשל

סיבות אפשריות:

- התהליך `PayPlus - Import Terminals & Pages` כבוי או שהחיבור שלו שבור.
- `MyTerminals` או `ListPaymentPages` החזירו תגובה ריקה או לא תקינה.
- אי-התאמת סביבה בין התצורה לבין החיבור.

פעולות:

- לוודא שהתהליך פעיל ומשתמש בחיבור תקין.
- להריץ מחדש את הייבוא משלב "מסופים ועמודי תשלום" באשף ההתקנה (או מקישור הייבוא-מחדש במרכז הניהול).
- לוודא ששורות מופיעות ב-`alex_payplus_terminal` וב-`alex_payplus_paymentpage`, לפי מפתח סביבה + UID.
- לזכור שהייבוא אידמפוטנטי: הרצה חוזרת מבצעת upsert לפי (סביבה + UID) ומשמרת שדות עסקיים/מדיניות.

## אין מסוף או עמוד ברירת מחדל

סיבות אפשריות:

- שלב האימות לא הושלם, ולכן אף רשומה אינה מסומנת ב-`alex_isdefault`.
- ברירת מחדל בוטלה ולא נבחרה מחדש.

פעולות:

- להריץ מחדש את שלב האימות ולבחור מסוף ברירת מחדל ואת עמוד התשלום ברירת המחדל שלו. פעולה זו כותבת `alex_terminaluidref` / `alex_paymentpageuidref` על התצורה ומסמנת `alex_isdefault = true`.
- מותר רק מסוף ברירת מחדל אחד לכל סביבה ועמוד ברירת מחדל אחד לכל מסוף + סוג תהליך; הפלאגינים `EnforceSingleDefaultTerminal` ו-`EnforceSingleDefaultPage` מבטלים את ברירת המחדל הקודמת אוטומטית.

## ייבוא סוגי מסמכים חוסם את סיום ההתקנה

סימפטומים:

- שלב האימות אינו מתקדם לשלב הסיום.
- ההתקנה נותרת בשלב האימות מכיוון שייבוא סוגי המסמכים נכשל או פג בזמן.

סיבות אפשריות:

- ייבוא סוגי המסמכים הוא שלב חובה וחוסם; ההתקנה אינה יכולה להסתיים עד שהוא מצליח.
- תהליך הייבוא כבוי, או שהחיבור שלו שבור.
- התהליך השתמש בביטוי inline לא תקין `filter()`.

פעולות:

- לוודא שתהליך ייבוא סוגי המסמכים פעיל ושהחיבור שלו תקין, ואז ללחוץ שוב על "הרץ אימות".
- ל-Power Automate אין פונקציית ביטוי `filter()`; יש להשתמש בפעולת **Filter array (Query)** לסינון מערכים.
