# מודל נתונים מוצע ב-Dataverse (לא מיושם)

## תחום המסמך

מסמך זה הוא הצעה עתידית בלבד למודל נתונים. הטבלאות שמתוארות כאן אינן קיימות כרגע ב-artifacts של המחבר ואינן נוצרות על ידי הריפו הזה.

יש להשתמש במסמך רק אם יישום עתידי יחליט להוסיף מעקב תשלומים ב-Dataverse, התאמות, תמיכה, ביקורת או רשומת בקשת תשלום עסקית.
