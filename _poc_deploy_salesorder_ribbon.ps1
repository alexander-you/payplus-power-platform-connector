# Deploy Sales Order PayPlus command bar resources and RibbonDiffXml to the Sales extension solution.
...
$root = $PSScriptRoot
$ribbonFile = Join-Path $root 'ribbon\salesorder\RibbonDiff.xml'
$work = Join-Path $root '_tmp_salesorder_ribbon'
$exportZip = Join-Path $work 'sales_ext_export.zip'
$importZip = Join-Path $work 'sales_ext_salesorder_ribbon.zip'
...
function ConvertTo-ODataEscapedString([string]$value) {
...
function Set-WebResource([string]$name, [string]$file, [int]$type, [string]$displayName) {
    ...
    $body = @{
        ...
        content = $content
        description = 'PayPlus Sales command bar resource.'
...
Add-EntityToSolutionIfNeeded 'salesorder'
...
$targetFolder = Join-Path $entitiesFolder 'SalesOrder'
if (-not (Test-Path -LiteralPath $targetFolder)) {
    $targetFolder = Get-ChildItem -LiteralPath $entitiesFolder -Directory | Where-Object { $_.Name -ieq 'salesorder' -or $_.Name -ieq 'Order' } | Select-Object -First 1 -ExpandProperty FullName
}
if (-not $targetFolder -or -not (Test-Path -LiteralPath $targetFolder)) {
    throw "Unpacked solution does not contain a SalesOrder entity folder. Is salesorder in $solution?"
}
...
Write-Host ''
Write-Host 'DONE. Sales Order PayPlus command bar draft was imported and published.'
Write-Host 'Open a Sales Order form or Sales Order view and hard refresh if the command bar cache is stale.'
